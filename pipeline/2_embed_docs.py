...
def doc_generator(path: str, tokenizer, batch_size: int):
    ids, texts = [], []
...
def main(cfg: DictConfig) -> None:
    # Setup logger -------------------------------------------------------------
    setup_logger(logger, dir=cfg.general.logs_dir, filename="2_embed_docs.log")
    ...
    # I/O Paths ================================================================
    # Input
    documents_path = join_path(cfg.dataset.data_dir, "collection.jsonl")

    # Output
    os.makedirs(cfg.general.data_dir, exist_ok=True)
    embeddings_path = join_path(cfg.general.data_dir, "embeddings.h5")
    mapping_path = join_path(cfg.general.data_dir, "doc_emb_map.jsonl")
    ...
    n_docs = count_lines(documents_path)
    pbar = tqdm(
        total=n_docs,
    ...
    with torch.cuda.amp.autocast(), torch.no_grad(), h5py.File(
        ...
        for doc_ids, input_ids, attention_mask in doc_generator(
            documents_path, tokenizer=tokenizer, batch_size=64
        ):
            # Compute embeddings -----------------------------------------------
            ...
            dset.resize(new_offset, axis=0)
            dset[offset:new_offset] = embeddings.detach().cpu().numpy()
