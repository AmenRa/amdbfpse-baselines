...
def write_list(
    x: list, path: Union[str, Path], encoding: str = "utf-8"
) -> None:
    with open(path, "w", encoding=encoding) as f:
        f.write("\n".join(x))
...
def write_csv(x: list[dict], path: Union[str, Path], encoding: str = "utf-8"):
...
def read_csv(path: Union[str, Path], encoding: str = "utf-8") -> list[dict]:
    with open(path, "r", encoding=encoding) as f:
        x = list(csv.DictReader(f, skipinitialspace=True))
    return x
