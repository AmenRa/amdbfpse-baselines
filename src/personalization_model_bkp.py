...
class PersonalizationModel(LightningModule):
    def __init__(
        ...
        language_model: str = "huawei-noah/TinyBERT_General_4L_312D",
    ...
    def interpolate_query_user(self, Q_emb: Tensor, U_emb: Tensor) -> Tensor:
        return (
            1 - self.personalization_weight
        ) * Q_emb + self.personalization_weight * U_emb
    ...
    def training_step(self, batch, batch_idx) -> float:
        ...
        # Interpolate user-query embeddings ------------------------------------
        interpolation_emb = self.interpolate_query_user(Q_emb, U_emb)

        # Compute positive scores ----------------------------------------------
        pos_scores = self.scoring_function(interpolation_emb, D_pos_emb)

        # HARD NEGATIVES =======================================================
        # Compute hard negative scores -----------------------------------------
        neg_scores = self.scoring_function(interpolation_emb, D_neg_emb)
    ...
    def validation_step(self, batch, batch_idx) -> float:
        ...
        # Compute scores -------------------------------------------------------
        scores = self.listwise_scoring_function(interpolation_emb, D_emb)
